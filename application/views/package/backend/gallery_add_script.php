...
$('.summernote').summernote({
  ...
  //--------------------------------------
    function Addimg() {
        ...
        var type = $('#type').val();
        var postData = new FormData($("#galleryForm")[0]);
         if(category == '0'){
           swal(
                    {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('PackageCMS/addimggallery') ?>',
            processData: false,
            contentType: false,
            data: postData,
            success: function (data, status) {
                ...
                if (status == 'success') {
                    ...
                    setTimeout(function () {
                        window.location.href = "<?php echo base_url('PackageCMS/GalleryAdd/') ?>" +type+'/'+ currentID;
                    }, 2000);
                } else {
                    swal({
    ...
    function  loadImages(ProID) {
        $.post('<?php echo base_url('PackageCMS/loadImggallery') ?>', {ProID: ProID}, function (data) {
            $('#showImage').empty();
            $('#showImage').html(data);
        });
    }
    ...
    var type = $('#type').val();
        ...
        if ((changeValue == '')) {
            ...
            $.post('<?php echo base_url('PackageCMS/updateOrdergallery') ?>', {dataID: dataID, FieldName: FieldName, changeValue: changeValue});
             setTimeout(function () {
                        window.location.href = "<?php echo base_url('PackageCMS/GalleryAdd/') ?>" +type+'/'+ currentID;
                    }, 2000);
        }
    ...
    function comfirmDelete(DataID, fileType, FileName) {
        ...
        swal({
            ...
            $.post('<?php echo base_url('PackageCMS/deletegallery') ?>', {DataID: DataID, fileType: fileType, FileName: FileName}, function (data) {
                console.log(data);
                if (data == '1') {
                    swal({
                        title: 'Deleted !',
            ...
            swal({
                ...
                $.post('<?php echo base_url('PackageCMS/updateThis1') ?>', {name: name,min:min,max:max, Adult:Adult,Child:Child,currentID: dataID}, function (data) {
                    if (data > 0) {
                        ...
                        setTimeout(function () {
                            window.location.href = "<?php echo base_url('PackageCMS/GalleryAdd/') ?>" + packageID;}, 2000);
                    } else {
                        swal({
 ...
 setTimeout(function(){ window.location.href = "<?php echo base_url('PackageCMS/GalleryAdd/')?>"+currentID; }, 2000);
