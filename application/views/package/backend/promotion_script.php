...
<script type="text/javascript">
		   ...
		   $.post('<?php echo base_url('PackageCMS/deleteData')?>' , { dataID : dataID , table : table }, 
			function(data){
				if(data==1){ 
                    ...
                    setTimeout(function(){ window.location.href = "<?php echo base_url('PackageCMS/promotion')?>"; }, 2000);
				}else{
					swal({
    ...
    function setShow_onWeb(dataID, val2, table) {
        ...
        $.post('<?php echo base_url('PackageCMS/set_ShowOnWeb') ?>', {dataID: dataID, check: check, table: table}, function (data) {
          ...
          //----------------------
    function setShow_index(dataID, val2, table) {
        var changeCheckbox = document.querySelector('.js-check-change');
        var x = changeCheckbox.checked;
        if (val2 == '0') {
            var check = '1';
        ...
        $.post('<?php echo base_url('PackageCMS/setShow_index') ?>', {dataID: dataID, check: check, table: table}, function (data) {
            if (data == 1) {
                $('#chc' + dataID).val(check);
                swal({
                    title: 'Edit data successfully.',
                    //text: 'You clicked the button!',
                    type: 'success',
