...
<script type="text/javascript">
    ...
    "search" : false
     ...
     function Add() {
        ...
        if (name == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url('PackageCMS/addcate') ?>',
                processData: false,
                contentType: false,
                data: postData,
                success: function (data, status) {
                    ...
                    if (status == 'success') {
                        ...
                        setTimeout(function () {
                            window.location.href = "<?php echo base_url('PackageCMS/Category_Palmthien') ?>" ;
                        }, 2000);
                    } else {
                        swal({
    ...
    //----------------------------------
   function  loadSeason() {
        $.post('<?php echo base_url('PackageCMS/loadcate') ?>', {}, function (data) {
    ...
    function updateOrder(dataID,FieldName,changeValue){
		...
		console.log('returnValue:-'+returnValue)
		 if((changeValue=='')){
			...
			$.post('<?php echo base_url('PackageCMS/updateOrderCate')?>',{ dataID:dataID , FieldName:FieldName , changeValue:changeValue }
				  ,function(data){ 
						if(data==1){
										swal({
                    ...
                    setTimeout(function(){ window.location.href = "<?php echo base_url('PackageCMS/Category_Palmthien')?>"; }, 2000);
