...
<script type="text/javascript">
    ...
    "search" : false
     ...
     function Add() {
        ...
        if (name == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url('PackageCMS/addcategallery') ?>',
                processData: false,
                contentType: false,
                data: postData,
                success: function (data, status) {
                    ...
                    if (status == 'success') {
                        ...
                        setTimeout(function () {
                            window.location.href = "<?php echo base_url('PackageCMS/Category_AndathienGallery') ?>" ;
                        }, 2000);
                    } else {
                        swal({
    ...
    //----------------------------------
   function  loadSeason() {
        $.post('<?php echo base_url('PackageCMS/loadcategallery/2') ?>', {}, function (data) {
    ...
    function updateOrder(dataID,FieldName,changeValue){
		...
		console.log('returnValue:-'+returnValue)
		 if((changeValue=='')){
			...
			$.post('<?php echo base_url('PackageCMS/updateOrderCategallery')?>',{ dataID:dataID , FieldName:FieldName , changeValue:changeValue }
				  ,function(data){ 
						if(data==1){
										swal({
                    ...
                    setTimeout(function(){ window.location.href = "<?php echo base_url('PackageCMS/Category_AndathienGallery')?>"; }, 2000);
    ...
    function updateThis(dataID) {
        ...
        if (name == '') {
            ...
            swal({
                ...
                $.post('<?php echo base_url('PackageCMS/updategallery') ?>', {name: name, currentID: dataID}, function (data) {
                    if (data > 0) {
                        $('#name').val('');
                        swal({
